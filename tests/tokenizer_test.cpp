...
TEST_CASE("replace punctuation with spaces") {
    SUBCASE("Empty String") {
        std::string input;
        std::string expected;
        auto result = input | tokenizer::replace_punctuation_with_spaces | tokenizer::to_string;
...
TEST_CASE("replace non-printable characters with spaces") {
    SUBCASE("Empty String") {
        std::string input;
        std::string expected;
        auto result = input | tokenizer::replace_non_printable_characters_with_spaces | tokenizer::to_string;
        CHECK_EQ(result, expected);
    }
...
TEST_CASE("sanitize") {
    SUBCASE("Combination of Punctuation and Non-Printable Characters") {
        std::string input = "Hello,\nWorld!";
        std::string expected = "hello  world ";
        auto result = input | tokenizer::sanitize | tokenizer::to_string;
        CHECK_EQ(result, expected);
    }
